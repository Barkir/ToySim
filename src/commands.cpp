...
uint32_t sign_extend(uint32_t value, uint32_t bit_length) {
    ...
    if (sign_bit) {
        uint32_t mask = (uint32_t) (1 << bit_length) - 1; // be careful here
        return value | (~mask);
    } else {
        uint32_t mask = (uint32_t) (1 << bit_length) - 1; // be careful here
        return value & mask;
...
void spuDump(SPU& spu) {
    ...
    for (int i = 0; i < 32; i++) {
        ...
        if (spu.regs[i] != 0)
            fprintf(stdout, GREEN "%-5u\t" RESET, spu.regs[i]);
        else
            fprintf(stdout, RED "%-5u\t" RESET, spu.regs[i]);
    }
    fprintf(stdout, BLUE "║\n╚═══════════════════════════════╝\n" RESET);
    fprintf(stdout, YELLOW "---------------------------------\n" RESET);
}
...
//  -----------------------------------
    ...
    spu.regs[rd] = cbit(spu.regs[rs1], spu.regs[rs2]);
    spu.pc += PC_INC;
}
